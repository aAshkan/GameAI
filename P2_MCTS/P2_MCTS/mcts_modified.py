#Name: Amir Ashkan Nourkhalaj
#Partner: Koosha Sheyvani
...
def rollout(state):
    ...
    #The AI will try to capture the first and second (depends on the counter) squares before opponent

    counter = 2 # number of times to attempt
    while not state.is_terminal():
        mList = list(state.legal_moves)

        if counter > 0:    #do it only for the first two times
            m = optimizer(mList, state.player_turn, state)
            #reward += 1
            #print("reward: ", reward)
            state.apply_move(m)
            counter -= 1
            ...
            state.apply_move(choice(mList))     #continue with the normal rollout process
    ...
    while not state.is_terminal():
        #print ("asd")
        mList = list(state.legal_moves)
        if counter > 0:    #do it only for specific number of times
            for turn in mList:
                copy = state.copy() #making multiple copies
                copy.apply_move(turn)
                if copy.player_turn == state.player_turn:     #making sure that the AI hasn't won 
                    m = turn
                    #reward += 1
                    #print("reward:", reward)
            m = choice(mList)
            ...
            state.apply_move(choice(mList))
    #pass
 
    """

def optimizer(mList, identity, state):
    for turn in mList:
        copy = state.copy()                 #making multiple copies
        copy.apply_move(turn)

        if copy.player_turn == identity:    #make sure it's not the reward turn for AI
            return turn

    return choice(mList)                    #return a random move in worst case scenario
